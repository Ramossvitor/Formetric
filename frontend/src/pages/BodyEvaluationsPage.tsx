...
const yearsAgo = (years: number) => { const date = new Date(); date.setFullYear(date.getFullYear() - years); return new Date(date.getTime() - date.getTimezoneOffset() * 60_000).toISOString().slice(0, 10) }

function EvaluationCard({ evaluation, selected, onSelect }: { evaluation: BodyEvaluationSummary; selected: boolean; onSelect: () => void }) {
  ...
  return <article className="body-evaluation-card surface-card">
    ...
    <Link aria-label={`Abrir ${version.title}`} className="body-evaluation-main" to={`/progress/evaluations/${evaluation.id}`}>
      <time dateTime={version.assessmentDate}><strong>{formatBodyDate(version.assessmentDate)}</strong><small>v{version.versionNumber}</small></time>
...
export function BodyEvaluationsPage() {
  const initial = useMemo<Range>(() => ({ from: yearsAgo(5), to: today() }), [])
