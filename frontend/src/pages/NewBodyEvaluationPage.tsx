...
export function NewBodyEvaluationPage() {
  ...
  return <main id="conteudo"><header className="page-heading body-heading"><div><p className="eyebrow"><Link to="/progress/evaluations">Avaliações</Link> / nova</p><h1>Nova avaliação corporal</h1><p className="heading-copy">Registre os dados e revise o snapshot antes de salvar.</p></div></header>{profile.isPending ? <div className="catalog-state" role="status"><span className="route-spinner" /><p>Preparando formulário…</p></div> : <><BodyEvaluationForm defaultAssessmentDate={today} error={create.error} onCancel={() => navigate('/progress/evaluations')} onSubmit={(input) => create.mutate(input)} pending={create.isPending} profileSuggestion={profile.data ? { birthDate: profile.data.birthDate, formulaSex: profile.data.formulaSex } : undefined} />{profile.isError ? <p className="body-profile-warning" role="note">Não foi possível carregar sugestões do perfil. Confirme idade e sexo de fórmula manualmente.</p> : null}</>}</main>
}
