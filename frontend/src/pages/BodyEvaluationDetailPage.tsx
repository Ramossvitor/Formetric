...
export function BodyEvaluationDetailPage() {
  const { today, locale, timeZone } = useProfileTimeContext(); const { id = '' } = useParams(); const [editingVersion, setEditingVersion] = useState<BodyEvaluationVersion | null>(null); const [viewingVersionId, setViewingVersionId] = useState<string | null>(null); const queryClient = useQueryClient(); const showToast = useToast(); const evaluation = useQuery(bodyEvaluationQuery(id))
  ...
  const archive = useMutation({ mutationFn: (restore: boolean) => restore ? restoreBodyEvaluation(id, evaluation.data!.identityVersion) : archiveBodyEvaluation(id, evaluation.data!.identityVersion), onSuccess: async (_result, restored) => { setEditingVersion(null); await queryClient.invalidateQueries({ queryKey: bodyEvaluationsQueryKey }); showToast({ message: restored ? 'Avaliação restaurada.' : 'Avaliação arquivada.', action: { label: 'Desfazer', onAct: () => archive.mutate(!restored) } }) } })
